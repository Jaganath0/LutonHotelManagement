...
public class BLUser {
	...
	public void setUser1(User user) throws InputException {
		try {
			if(this.validateUser1(user)) {
				this.user = user;
			}
		}catch(InputException ex) {
			throw ex;
	...
	private boolean validateUser1(User user) throws InputException {
	...
	private boolean validateUser(User user) throws InputException {
		...
		if(user.getMobile() == null || user.getMobile().length() == 0) {
			throw new InputException("User mobile cannot be empty.");
		}
